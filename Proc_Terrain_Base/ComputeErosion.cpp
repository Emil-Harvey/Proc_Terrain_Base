...
ComputeErosion::ComputeErosion(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"shaders/erosion_cs.cso", NULL);
}

ComputeErosion::~ComputeErosion()
{
...
void ComputeErosion::initShader(const wchar_t* cfile, const wchar_t* blank)
{
	loadComputeShader(cfile);
	createOutputUAV();
}

void ComputeErosion::createOutputUAV()
{
...
void ComputeErosion::setShaderParameters(ID3D11DeviceContext* dc, ID3D11ShaderResourceView* texture1, ShaderVariables* SVars)
{
...
void ComputeErosion::unbind(ID3D11DeviceContext* dc)
{
